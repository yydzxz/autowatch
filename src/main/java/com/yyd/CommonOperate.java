...
public class CommonOperate {
    ...
    public static void 上划或者下划(int 总共要划动的距离, int 分几次划动完成, int 每次划动耗时单位毫秒, int 划动后最少停止时间毫秒, int offset, boolean 上划) throws IOException, InterruptedException {
        ...
        if(分几次划动完成 > 1){
            ...
            前n减1次划动距离最大为 = Math.min(前n减1次划动距离最大为, 上下划动的最长距离);
            log.info("前n减1次划动距离最大为: {}", 前n减1次划动距离最大为);
            前n减1次划动距离最小为 = (总共要划动的距离 - 上下划动的最长距离)/(分几次划动完成 - 1);
            前n减1次划动距离最小为 = Math.max(前n减1次划动距离最小为, 上下划动的最短距离);
            log.info("前n减1次划动距离最小为: {}", 前n减1次划动距离最小为);
...
//                坐标数组[i + 1][1] =  坐标数组[i][1] - 随机分配的距离;
                ...
                log.info("随机分配的距离: {}", 随机分配的距离);
                坐标数组[i][1] = 屏幕y轴最小可用坐标 + 随机分配的距离 + (屏幕y轴最大可用坐标 - 2 * 随机分配的距离) * random.nextDouble();
                坐标数组[i + 1][1] =  坐标数组[i][1] - 随机分配的距离;
